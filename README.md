...
### 自进化的 Agent OS —— 你搭建的每一个 Agent 都会学习、记忆、进化
...
**别人的 Agent 用完就忘。我们的 Agent 会学习、会记忆、会进化。**

OPC Agent 是一个**自进化的 Agent OS**：从 `opc init` 创建，到 `opc chat` 对话，到 `opc studio` 可视化管理。<br>内置 learn → recall → evolve 闭环，让 Agent **越用越聪明**——不是每次从零开始，而是持续积累和进化。<br>同时，OPC 项目本身也在自进化——社区驱动 + 每天自动迭代，代码也在不断进化。
...
OPC Agent 的核心设计哲学是**自进化 + 全渠道 + 工位模板 + 生产工具链四件套**。与大多数 Agent 框架关注"如何编排多个 Agent 对话"不同，OPC 更关注的是：一个 Agent 如何在真实生产环境中**持续自进化、随处触达、开箱即用**。这决定了我们在架构选择上的一系列差异。
...
### The Self-Evolving Agent OS — Every Agent You Build Learns, Remembers, and Evolves
...
**Other Agents forget everything after each session. Ours learn, remember, and evolve.**

OPC Agent is a **self-evolving Agent OS**: from `opc init` to create, `opc chat` to converse, `opc studio` to manage visually.<br>The built-in learn → recall → evolve loop makes your Agent **smarter over time** — not starting from scratch, but continuously accumulating and evolving knowledge.<br>And the OPC project itself is also self-evolving — community-driven with daily iterations, the codebase keeps getting better.
...
OPC Agent's core design philosophy centers on **Self-Evolution + Omnichannel + Role Templates + Production Toolchain (the "Four-Piece Suite")**. While most agent frameworks focus on "how to orchestrate multi-agent conversations," OPC focuses on how a single agent can **continuously self-evolve, reach users everywhere, and ship to production out of the box**. This fundamental difference drives our architectural choices.
